...
export const TasksView: React.FC<TasksViewProps> = ({
  ...
  return (
    <div className="pb-24 pt-2 px-4 max-w-md mx-auto space-y-4 font-sans text-black dark:text-gray-100 bg-white dark:bg-gray-900 min-h-screen transition-colors duration-200">
      {/* Target / Goals Banner */}
      <div className="bg-accent border-2 border-black rounded-2xl p-4 shadow-[3px_4px_0px_0px_rgba(0,0,0,1)] relative flex justify-between items-center">
          ...
          return (
            <button
              ...
              className={`border-2 border-black rounded-2xl p-3 shadow-[3px_4px_0px_0px_rgba(0,0,0,1)] relative flex flex-col justify-between h-32 text-left transition-all ${
                isSelected ? 'bg-accent' : 'bg-white dark:bg-gray-800'
              }`}
            >
      ...
      {/* Task List Header */}
      <div className="pt-2">
        <h3 className="font-bold text-sm text-[#1E293B] dark:text-gray-100">
      ...
      <div className="space-y-3">
        {filteredTasks.length === 0 ? (
          <div className="bg-white dark:bg-gray-800 border-2 border-black rounded-2xl p-8 text-center shadow-[3px_4px_0px_0px_rgba(0,0,0,1)] min-h-[170px] flex flex-col items-center justify-center space-y-3">
            <span className="text-3xl text-amber-400">✦</span>
            <p className="font-bold text-[#1E293B] dark:text-gray-100 text-sm leading-relaxed px-4">
          ...
          filteredTasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className={`bg-white dark:bg-gray-800 border-2 border-black rounded-2xl p-3.5 shadow-[3px_4px_0px_0px_rgba(0,0,0,1)] relative transition-all ${
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-2">
                      <h4 className={`text-sm font-bold leading-snug line-clamp-2 ${
                        task.completed ? 'line-through text-gray-400' : 'text-black dark:text-gray-100'
                      }`}>
                        {task.title}
                      </h4>
                      <button
