...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#FCF9F8] border-t-[3px] border-[#1A1A1A] py-2 px-3 shadow-[0_-3px_0px_#1A1A1A]">
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all duration-150 relative ${
                isActive
                  ? 'scale-105 font-bold text-[#1A1A1A]'
                  : 'text-gray-600 hover:text-black font-medium opacity-80'
              }`}
            >
              {/* Highlight Circle for Active Tab */}
              {isActive ? (
                <div className="w-10 h-10 rounded-full bg-[#FFE66D] doodle-border-sm doodle-shadow-sm flex items-center justify-center mb-0.5 text-black">
                  {tab.icon}
                </div>
              ) : (
