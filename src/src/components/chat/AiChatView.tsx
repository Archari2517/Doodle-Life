  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      text: user.language === 'th'
        ? `สวัสดีครับคุณ ${user.name}! 🌿 ผมคือ Doodle AI ผู้ช่วยส่วนตัวของคุณ\nผมได้เชื่อมโยงช่วงเวลาพลังงานพีค (${user.peakHours}) และเป้าหมาย ${goals.length} รายการของคุณแล้ว พร้อมช่วยวางแผนหรือให้คำแนะนำได้ทันทีครับ!`
  ...
  return (
    <div className="pb-28 pt-2 px-4 max-w-md mx-auto flex flex-col h-[calc(100vh-145px)]">
      ...
      <div className="flex-1 overflow-y-auto space-y-3.5 pr-1">
        {messages.map((msg) => (
          ...
          >
            <div className="flex items-end gap-1.5 max-w-[88%]">
              {msg.sender === 'ai' && (
                <div className="w-7 h-7 rounded-full bg-[#FFE66D] doodle-border-sm flex items-center justify-center text-xs font-black shrink-0 mb-1">
                  🤖
                </div>
              )}
              ...
              <div
                className={`p-3.5 doodle-border doodle-shadow-sm text-xs leading-relaxed whitespace-pre-wrap ${
                  msg.sender === 'user'
                    ? 'bg-[#1A1A1A] text-[#FFE66D] rounded-br-none'
              ...
              >
                ...
                {msg.quickAction && (
                  <button
                    ...
                    className="mt-3 w-full bg-[#FFE66D] hover:bg-[#ffd92a] text-black doodle-border-sm py-2 px-3 font-extrabold text-[11px] doodle-btn flex items-center justify-center gap-1.5 shadow-[2px_2px_0px_#000]"
                  >
        ...
        {isLoading && (
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-[#FFE66D] doodle-border-sm flex items-center justify-center text-xs font-black">
              🤖
            </div>
      ...
      <div className="flex gap-1.5 overflow-x-auto no-scrollbar py-2 shrink-0">
        {quickChips.map((chip, idx) => (
          <button
            ...
            className="bg-white hover:bg-[#FFE66D] doodle-border-sm px-2.5 py-1 text-[11px] font-extrabold shrink-0 doodle-shadow-sm doodle-btn"
          >
            {chip.label}
          </button>
        ))}
      ...
      >
        ...
        <button
          ...
          className="bg-[#1A1A1A] text-[#FFE66D] px-4 doodle-border border-[#1A1A1A] font-extrabold text-xs doodle-btn flex items-center justify-center shrink-0"
        >
